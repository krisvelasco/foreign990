# Location Vectors for Data Cleaning

# Activities
noregion <- activities_cln %>%
...
test <- activities_cln  %>%
  ...
  filter(dest_europe == 1)
    ...
    dest_asia_pacific = case_when(
      f_location %in% Europe ~ 0,
      !f_location %in% Europe ~ dest_asia_pacific,      
    ),
    dest_americas = case_when(
      f_location %in% Europe ~ 0,
      !f_location %in% Europe ~ dest_americas,     
    ),
    dest_europe = case_when(
      f_location %in% Europe ~ 1,
      !f_location %in% Europe ~ dest_europe,    
    ),
    dest_multi = case_when(
      f_location %in% Europe ~ 0,
      !f_location %in% Europe ~ dest_multi,      
    ),
    # Asia
    dest_africa = case_when(
      f_location %in% Asia ~ 0,
      !f_location %in% Asia ~ dest_africa,
    ),
    dest_asia_pacific = case_when(
      f_location %in% Asia ~ 1,
      !f_location %in% Asia ~ dest_asia_pacific,      
    ),
    dest_americas = case_when(
      f_location %in% Asia ~ 0,
      !f_location %in% Asia ~ dest_americas,     
    ),
    dest_europe = case_when(
      f_location %in% Asia ~ 0,
      !f_location %in% Asia ~ dest_europe,    
    ),
    dest_multi = case_when(
      f_location %in% Asia ~ 0,
      !f_location %in% Asia ~ dest_multi,      
    ),
    # Asia and Africa
    dest_africa = case_when(
      f_location %in% AsiaAfrica ~ 1,
      !f_location %in% AsiaAfrica ~ dest_africa,
    ),
    dest_asia_pacific = case_when(
      f_location %in% AsiaAfrica ~ 1,
      !f_location %in% AsiaAfrica ~ dest_asia_pacific,      
    ),
    # Africa and Americas
    dest_africa = case_when(
      f_location %in% AfricaAmericas ~ 1,
      !f_location %in% AfricaAmericas ~ dest_africa,
    ),
    dest_americas = case_when(
      f_location %in% AfricaAmericas ~ 1,
      !f_location %in% AfricaAmericas ~ dest_asia_pacific,      
    ),
    # Africa and Europe
    dest_africa = case_when(
      f_location %in% AfricaEurope ~ 1,
      !f_location %in% AfricaEurope ~ dest_africa,
    ),
    dest_europe = case_when(
      f_location %in% AfricaEurope ~ 1,
      !f_location %in% AfricaEurope ~ dest_europe,      
    ),
    # Asia and Americas
    dest_asia_pacific = case_when(
      f_location %in% AsiaAmericas ~ 1,
      !f_location %in% AsiaAmericas ~ dest_asia_pacific,
    ),
    dest_americas = case_when(
      f_location %in% AsiaAmericas ~ 1,
      !f_location %in% AsiaAmericas ~ dest_americas,      
    ),
    # Asia, Americas, and Africa
    dest_asia_pacific = case_when(
      f_location %in% AsiaAmericasAfrica ~ 1,
      !f_location %in% AsiaAmericasAfrica ~ dest_asia_pacific,
    ),
    dest_americas = case_when(
      f_location %in% AsiaAmericasAfrica ~ 1,
      !f_location %in% AsiaAmericasAfrica ~ dest_americas,      
    ),
    dest_africa = case_when(
      f_location %in% AsiaAmericasAfrica ~ 1,
      !f_location %in% AsiaAmericasAfrica ~ dest_africa,      
    ),
    # Asia, Europe and Africa
    dest_asia_pacific = case_when(
      f_location %in% AsiaEuropeAfrica ~ 1,
      !f_location %in% AsiaEuropeAfrica ~ dest_asia_pacific,
    ),
    dest_europe = case_when(
      f_location %in% AsiaEuropeAfrica ~ 1,
      !f_location %in% AsiaEuropeAfrica ~ dest_europe,      
    ),
    dest_africa = case_when(
      f_location %in% AsiaEuropeAfrica ~ 1,
      !f_location %in% AsiaEuropeAfrica ~ dest_africa,      
    ),
    # Asia, Europe, Africa, and Americas
    dest_asia_pacific = case_when(
      f_location %in% AsiaEuropeAfricaAmericas ~ 1,
      !f_location %in% AsiaEuropeAfricaAmericas ~ dest_asia_pacific,
    ),
    dest_europe = case_when(
      f_location %in% AsiaEuropeAfricaAmericas ~ 1,
      !f_location %in% AsiaEuropeAfricaAmericas ~ dest_europe,      
    ),
    dest_africa = case_when(
      f_location %in% AsiaEuropeAfricaAmericas ~ 1,
      !f_location %in% AsiaEuropeAfricaAmericas ~ dest_africa,      
    ),
    dest_americas = case_when(
      f_location %in% AsiaEuropeAfricaAmericas ~ 1,
      !f_location %in% AsiaEuropeAfricaAmericas ~ dest_americas,      
    ),
    # Europe and Americas
    dest_europe = case_when(
      f_location %in% EuropeAmericas ~ 1,
      !f_location %in% EuropeAmericas ~ dest_europe,      
    ),
    dest_americas = case_when(
      f_location %in% EuropeAmericas ~ 1,
      !f_location %in% EuropeAmericas ~ dest_americas,      
    ),
    # Europe and Asia
    dest_europe = case_when(
      f_location %in% EuropeAsia ~ 1,
      !f_location %in% EuropeAsia ~ dest_europe,      
    ),
    dest_asia_pacific = case_when(
      f_location %in% EuropeAsia ~ 1,
      !f_location %in% EuropeAsia ~ dest_asia_pacific,      
    ),
    # Asia, Europe and Americas
    dest_asia_pacific = case_when(
      f_location %in% EuropeAsiaAmericas ~ 1,
      !f_location %in% EuropeAsiaAmericas ~ dest_asia_pacific,
    ),
    dest_europe = case_when(
      f_location %in% EuropeAsiaAmericas ~ 1,
      !f_location %in% EuropeAsiaAmericas ~ dest_europe,      
    ),
    dest_americas = case_when(
      f_location %in% EuropeAsiaAmericas ~ 1,
      !f_location %in% EuropeAsiaAmericas ~ dest_americas,      
    )
  )
...
# Asia, Europe, Africa
#------------------------------
AsiaEuropeAfrica <- c("europe/s. africa/middle east",
...
#------------------------------
AsiaEuropeAfricaAmericas <- c("europe, africa, asia, central america, latin america, kenya",
...
# Africa, Americas
#------------------------------
AfricaAmericas <- c("africa & latin america",
...
#------------------------------
AsiaAmericasAfrica <- c("africa, asia & south america",
...
# Asia, Americas
#------------------------------
AsiaAmericas <- c("americas/israel",
                  "india & mexico",
...
#------------------------------
EuropeAsiaAmericas <- c("europe, asia and north america (not the united states)",
