...
nonprofits_analysis <- read_csv("/Users/srojascabal/Desktop/000_f990_data/analytical_sample_220725.csv",
     col_types = cols(
      ein = col_character(),
  ...
  mutate(
    ...
    log_state_religiosity = log(rel_orgs),
    log_excempt_orgs = log(excempt_orgs)
  )
...
# 990/EZ filers
plot_excempt_orgs <-
  ggplot(nonprofits_analysis, aes(x = excempt_orgs)) +
  geom_density() +
  labs(
    title = "Number of F990/EZ Filing Orgs"
  )

plot_log_excempt_orgs <-
  ggplot(nonprofits_analysis, aes(x = log_excempt_orgs)) +
  geom_density() +
  labs(
...
grid.arrange(plot_excempt_orgs, plot_log_excempt_orgs,
             ncol = 2) 
```
...
model2_fe <- felm(log_frgnXpns_2013_100k ~ anti_lgbtq + gov_republican + log_gdp_2012 + ind_yearMrgEq_rtrn + log_excempt_orgs + log_state_religiosity + log_totalXpns_2013_100k |
                rtrn_state + ein |
                0 | 
                rtrn_state + ein,
                anti_nonprofits)
...
model4_fe <- felm(propFrgnXpns_2013_100k ~ anti_lgbtq + gov_republican + log_gdp_2012 + ind_yearMrgEq_rtrn + log_excempt_orgs + log_state_religiosity + log_totalXpns_2013_100k |
                rtrn_state + ein |
                0 | 
                rtrn_state + ein,
                anti_nonprofits)
